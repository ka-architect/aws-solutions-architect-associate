...
- **Elastic Load Balancer (ELB):** managed AWS service for load balancing, within a region
...
- Access logging can be enabled for ELB and exported to S3 as compressed files, disabled by default
- Access logs capture information about requests sent to LB (time, client IP, latencies, paths, and responses)
...
- Supports HTTP/HTTPS health check, gRPC
- Can route a request to a service based on content of request (Host field, Path URL, HTTP header, HTTP method, Query string, or Source IP)
- **Path-Based Routing:** routing traffic based on path URL to a specific target group
...
## Cross-Zone LB
- Ensures LB distributes evenly to instances in all AZ's
- If disabled, LB will just distribute evenly across AZ's not taking into consideration number of instances in each AZ (imbalance)
...
## ASG Lifecycle
- Starts when the Auto Scaling group launches an instance and puts it into service, ends when you terminate/ASG takes instance out of service and terminates
- Scale-Out lifecycle: Pending -> Pending:Wait -> Pending:Proceed -> InService
- Scale-In lifecycle: Terminating -> Terminating:Wait -> Terminating:Proceed -> Terminated
- **Lifecycle Hook:** added to ASG to perform custom actions when instances launch or terminate
    - ``autoscaling:EC2_INSTANCE_LAUNCHING`` perform action after Pending and move to Pending:Wait after action
    - ``autoscaling:EC2_INSTANCE_TERMINATING`` perform action after Terminating and move to Terminating:Wait after action
- Can export lifecycle action events to CloudWatch via Lambda for further action/analysis

## Attributes
- **Launch Template:** information on how to launch EC2 within your ASG (AMI, Type, Storage, Security, network, etc.)
    - one launch template for an ASG at a time
    - can’t modify a launch template after you’ve created it, must create a new one and then attach to ASG
...
- **Dynamic Scaliing Policies:** scaling based on dynamic situations
    ...
    - **Step Scaling:** triggered by alarm, based on a set of scaling adjustments, known as step adjustments
    - **Simple Scaling:** triggered by alarm, based on a single scaling adjustment
